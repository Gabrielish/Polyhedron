...
export function GameDataTab({ document, onDocumentChange }: { document: WorkspaceSyncDocument; onDocumentChange: (document: WorkspaceSyncDocument) => void }): React.JSX.Element {
  ...
  return <section className="game-data-panel">
    <div className="game-data-controls"><div className="game-data-search-row"><label className="search-field"><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title or description..." /></label><a className="game-data-open" href={current ? wikiUrl(current) : 'https://bg3.wiki/wiki/Weapons'} target="_blank" rel="noreferrer" title="Open selected entry on Baldur's Gate wiki" aria-label="Open selected entry on Baldur's Gate wiki">↗</a></div><div className="game-data-categories">{CATEGORIES.map((item) => <button key={item.value} type="button" className={category === item.value ? 'game-data-category active' : 'game-data-category'} onClick={() => { setCategory(item.value); setSelected(null) }}><CategoryIcon category={item.value} /> <span>{item.label}</span></button>)}</div></div>
