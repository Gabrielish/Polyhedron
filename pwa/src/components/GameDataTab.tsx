...
export function GameDataTab({ document, onDocumentChange }: { document: WorkspaceSyncDocument; onDocumentChange: (document: WorkspaceSyncDocument) => void }): React.JSX.Element {
  ...
  return <section className="game-data-panel">
    ...
    <div className="game-data-controls"><label className="search-field"><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title or description..." /></label><div className="game-data-categories">{CATEGORIES.map((item) => <button key={item.value} type="button" className={category === item.value ? 'game-data-category active' : 'game-data-category'} onClick={() => { setCategory(item.value); setSelected(null) }}><CategoryIcon category={item.value} /> <span>{item.label}</span></button>)}</div></div>
