...
export function DialogueNodesTab({ document, onDocumentChange }: { document: WorkspaceSyncDocument; onDocumentChange: (document: WorkspaceSyncDocument) => void }): React.JSX.Element {
  ...
  const items = useMemo<DialogueItem[]>(() => {
    ...
    for (const session of document.sessions) for (const entry of session.entries) {
      ...
      for (const [categoryId, fileId, dialogueId, nodeId] of groups) {
        ...
        const act = category === 'Act 1' ? (/AstralPlane|Monastery|UpperCreche|LowerCreche/i.test(files[fileId] ?? '') ? 'Act 1B' : 'Act 1') : category === 'Act 2' || category === 'Act 3' ? category : 'Global'
  ...
  const filtered = items.filter((item) => (act === 'Act 1' ? (item.act === subAct) : item.act === act) && item.name.toLocaleLowerCase().includes(query.toLocaleLowerCase()))
  ...
  return <section className="dialogue-panel">
    ...
    <div className="dialogue-acts">{ACTS.map((item) => <button key={item} type="button" className={act === item ? 'dialogue-act active' : 'dialogue-act'} onClick={() => { setAct(item); setSubAct(item === 'Act 1' ? 'Act 1' : item); setSelected(null) }}>{item}</button>)}</div><div className="dialogue-controls"><label className="dialogue-search"><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search dialogue node name..." /></label><button type="button" className="dialogue-open-button" title="Open online" aria-label="Open online" disabled={!selected} onClick={() => selected && window.open(`https://bg3.game-script.com/files/${encodeURIComponent(selected.name)}`, '_blank', 'noopener,noreferrer')}>↗</button></div>
