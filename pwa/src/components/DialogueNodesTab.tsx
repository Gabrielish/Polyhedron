...
export function DialogueNodesTab({ document, onDocumentChange }: { document: WorkspaceSyncDocument; onDocumentChange: (document: WorkspaceSyncDocument) => void }): React.JSX.Element {
  ...
  const items = useMemo<DialogueItem[]>(() => {
    ...
    for (const session of document.sessions) for (const entry of session.entries) {
      ...
      for (const [categoryId, fileId, dialogueId, nodeId] of groups) {
        ...
        const act = category === 'Act 1' ? (/AstralPlane|Monastery|UpperCreche|LowerCreche/i.test(files[fileId] ?? '') ? 'Act 1B' : 'Act 1') : category === 'Act 2' ? (/Intermezzo/i.test(files[fileId] ?? '') ? 'Act 2B' : 'Act 2') : category === 'Act 3' ? category : 'Global'
  ...
  const filtered = items.filter((item) => ((act === 'Act 1' || act === 'Act 2') ? (item.act === subAct) : item.act === act) && item.name.toLocaleLowerCase().includes(query.toLocaleLowerCase()))
