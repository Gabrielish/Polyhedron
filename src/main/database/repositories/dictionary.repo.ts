...
export class DictionaryRepository {
  ...
  bulkUpsert(rows: UpsertParams[]): void {
    ...
    const byUid = new Map<string, DictionaryEntry>()
    const bySourceWithoutUid = new Map<string, DictionaryEntry>()
    for (const entry of existing) {
      const sourceKey = dictionaryTextKey(swapped ? entry.textLanguage2 : entry.textLanguage1)
      if (entry.uid) byUid.set(entry.uid, entry)
      else bySourceWithoutUid.set(sourceKey, entry)
    }

    this.db.transaction((tx) => {
      for (const row of rows) {
        const values = this.toValues(row)
        const sourceKey = dictionaryTextKey(row.sourceText)
        ...
        if (match) {
          ...
          tx.insert(dictionary).values(values).run()
        }
      }
    })
  }
  ...
  private buildFilterWhere(filters: DictionaryFilters): SQL | undefined {
    ...
    if (text) {
      if (exactMatch) {
        conditions.push(
          sql`(
            lower(${dictionary.textLanguage1}) = ${text}
            or lower(${dictionary.textLanguage2}) = ${text}
            or lower(coalesce(${dictionary.uid}, '')) = ${text}
            or lower(coalesce(${dictionary.modName}, '')) = ${text}
          )`
        )
      } else {
        const pattern = `%${text}%`
