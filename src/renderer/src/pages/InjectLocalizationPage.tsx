      ...
      <div className={embedded ? 'w-full' : 'mx-auto w-full max-w-4xl'}>
        <div className="mb-7 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-neutral-500">
              Build the current translation and install it as the game&apos;s English.pak.
            </p>
          </div>
        </div>
        ...
        <div className="mb-5 rounded-xl border border-amber-500/20 bg-amber-500/5 p-4 text-sm text-neutral-300">
          <div className="mb-2 flex items-center gap-2 font-medium text-amber-300">
            <ShieldCheck size={16} /> Safe replacement
          </div>
          <p className="leading-6 text-neutral-400">
            The first injection renames the original English.pak to EnglishOld.pak.bak (so the game
            will not load the backup). Later injections keep that backup and replace only the active
            English.pak.
          </p>
        </div>
        ...
        {session.phase !== 'loaded' ? (
          ...
          <>
            <div className="mb-4 text-xs text-neutral-500">
              {session.entries.length.toLocaleString()} entries ready from{' '}
              {session.modName || 'the current XML'}.
            </div>
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              >
                <div className="mb-4 flex items-center justify-between">
                  ...
                  {running === 'windows' ? (
                    <span className="text-xs text-amber-300">Injecting…</span>
                  ) : (
