...
export function DictionaryPage(): React.JSX.Element {
  ...
  const filters = useMemo<DictionaryFilters>(
    ...
    [debouncedText, exactMatch, modName, sourceLang, targetLang]
  )
  ...
  const hasFilters = Boolean(text || exactMatch || modName || sourceLang || targetLang)
  ...
  return (
    <div className="flex h-full min-h-0 flex-col bg-[#0f1114] text-neutral-100">
      ...
      <div className="flex flex-wrap items-end gap-x-2 gap-y-3 border-b border-[#1f2329] bg-[#0f1114] px-4 py-3">
        ...
        <label className="inline-flex h-8 cursor-pointer items-center gap-2 rounded-md border border-[#1f2329] bg-[#131518] px-2.5 text-xs text-neutral-400 hover:text-neutral-200">
          <input
            type="checkbox"
            checked={exactMatch}
