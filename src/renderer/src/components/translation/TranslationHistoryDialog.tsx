...
export function TranslationHistoryDialog({
  ...
  const deleteEntry = (historyId: string) => {
    ...
    const entry = session.entries.find((item) =>
      item.history?.some((change) => change.id === historyId)
    )
  ...
  return (
    <ModalShell open title="History of changes" onClose={onClose} icon={<History size={16} />}>
      <div className="mb-3 rounded border border-[#1f2329] bg-[#0c0d0f] p-2 text-xs text-neutral-400">
        {source}
      </div>
          ...
          {[...visibleHistory].reverse().map((change) => (
            <div key={change.id} className="rounded border border-[#1f2329] bg-[#131518] p-3">
              <div className="mb-1 flex items-center justify-between gap-2 text-[10px] uppercase tracking-wide text-neutral-500">
                <span>
                  {change.kind === 'translation'
                    ? 'Translation'
                    : change.kind === 'gender'
                      ? `${change.variant ?? 'variant'} translation`
                      : change.kind === 'review'
                        ? 'Review status'
                        : 'Needs review'}
                </span>
                <span className="inline-flex items-center gap-2">
                  <time>{new Date(change.changedAt).toLocaleString()}</time>
                  <button
                    type="button"
                    onClick={() => deleteEntry(change.id)}
                    title="Delete history entry"
                    aria-label="Delete history entry"
                    className="rounded p-0.5 text-neutral-500 hover:bg-red-500/15 hover:text-red-300"
                  >
                    <X size={12} />
                  </button>
                </span>
              ...
              {change.previousValue?.trim() && (
                <div className="mt-1 border-t border-[#1f2329] pt-1 text-xs text-neutral-500">
