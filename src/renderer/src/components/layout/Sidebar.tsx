...
const NAV_GROUPS: NavGroupConfig[] = [
  {
    ...
    items: [
      ...
      { to: '/dictionary', icon: Database, labelKey: 'dictionary', kbd: 'Ctrl 2' },
  ...
  },
  {
    label: 'Workspace',
    icon: FolderSync,
    items: [{ to: '/workspace', icon: FolderSync, labelKey: 'workspace', kbd: '' }]
  }
]

const FOOTER_ITEMS: NavItemConfig[] = [
  { to: '/settings', icon: Settings, labelKey: 'settings', kbd: '' }
]
...
function NavItem({
  ...
  return (
    ...
    >
      ...
      <span className="shortcut-hint whitespace-nowrap font-mono text-[9px] text-neutral-600 opacity-0 transition-opacity duration-150 group-hover/sidebar:opacity-100">
        {kbd}
      </span>
    </NavLink>
  )
      ...
      <nav className="sidebar-nav flex-1 overflow-y-auto px-2 py-3">
        {NAV_GROUPS.map((group) => {
          const items = group.items.filter(
            (item) =>
              (item.to !== '/game-interface' || showGameInterface) &&
